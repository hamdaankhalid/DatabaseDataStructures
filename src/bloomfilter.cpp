...
DatabaseDataStructure::BloomFilter::BloomFilter(int size, std::vector<std::hash<std::string>> hashes): _bit_flowers(size, false), _hashers(hashes) {}

void DatabaseDataStructure::BloomFilter::add(std::string item) {
  for (auto hash_func: _hashers) {
    int hashed_inp = hash_func(item) % _bit_flowers.size();
